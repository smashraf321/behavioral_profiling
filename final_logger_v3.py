        ...
        if hf.if_in_depot(float(curr_lat),float(curr_lon),distance_total,RETURN_TO_DEPOT,vspeed2):
            ...
            if RETURN_TO_DEPOT:
                if FIRST_TIME_CAPTURE:
                    #capture start time
                    time_start_at_stop = time.time()
                    time_curr_at_stop = time_start_at_stop
                    time_spent_at_stop = 0.0
                    FIRST_TIME_CAPTURE = False
                if time_spent_at_stop > 15.0 and time_spent_at_stop < 20.0:
                    if file_open:
                        outfile_can.close()
                    file_open = False
                    #os.system("./final_upload.sh")
                    ...
                    STARTED_FROM_DEPOT = True
                time_curr_at_stop = time.time()
                time_spent_at_stop = time_curr_at_stop - time_start_at_stop
        else:
            print('R')
            ...
            if hf.geo_fence_start(float(curr_lat),float(curr_lon),distance,vspeed2,FIRST_TIME_START,CIRCULATOR):
                ...
                if time_spent_at_stop > 15.0 and time_spent_at_stop < 20.0:
                    if not NEW_DATA_START_LOC:
                        ...
                        distance = 0
                        # save previous file
                        # read name of prev file
                        outfile_name = open('current_file.txt','r')
                        filename = outfile_name.readline()
                        outfile_name.close()
                        # save it in file called previous file
                        outfile_name = open('previous_file.txt','w+')
                        print(filename.rstrip(),file = outfile_name)
                        outfile_name.close()
                        # save new file name
                        ...
                        #file_count += 1
                if time_spent_at_stop > 20.0:
                    # close existing (current) file and remove it
                    if file_open:
                        outfile_can.close()
                        print('Closed current file')
                    outfile_name = open('current_file.txt','r')
                    filename = outfile_name.readline()
                    outfile_name.close()
                    to_be_removed = "rm -f " + filename.rstrip()
                    os.system(to_be_removed)
                    # open previously logged file and start appending to it (there will be time stamp delay of 5 seconds but vehicle didn't move during then)
                    # this means we need current file and previous file, prev file will be tracked only when we come to first bus stop, not at depot
                    outfile_name = open('previous_file.txt','r')
                    filename = outfile_name.readline()
                    outfile_name.close()
                    outfile_can = open(filename.rstrip(),'a')
                    file_open = True
                time_curr_at_stop = time.time()
                time_spent_at_stop = time_curr_at_stop - time_start_at_stop
            else:
